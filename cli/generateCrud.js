...
async function generateEmptyCrud(entity) {
  ...
  const appFile = await fs.readFile('app.js', 'utf8');
  const newRouteImport = `const ${entity}Routes = require('./src/routes/${entity}/${entity}.route');`;
  ...
  await fs.writeFile('app.js', updatedAppFile);
}
